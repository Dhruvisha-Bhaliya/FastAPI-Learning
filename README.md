...
The backend is built using Python and FastAPI. It provides REST APIs that communicate between the React frontend and PostgreSQL database.
...
venv contains installed Python packages, while .env can contain sensitive configuration and credentials.
...
SQLAlchemy is the ORM used to communicate with PostgreSQL using Python objects and models.
...
Python → SQLAlchemy → Psycopg → PostgreSQL
...
--reload automatically restarts the server when backend source files are changed during development.
